...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
    ...
    var $ = layui.$
        ...
        ,TPL_HEADER = function(options){
            ...
            return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
                ...
                ,'<th{{# if(item2.hide===true){ }} class="cy-none" {{# } }} data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize){ }}data-unresize="true"{{# } }}>'
    ...
    Class.prototype.renderData = function(res, curr, count, sort){
        ...
        var that = this
            ...
            ,render = function(){ //后续性能提升的重点
                /**记录该table渲染前数据*/
                var _rowData= $.extend(true,[], res[options.response.dataName] || []);
                table.rowData[options.id || options.index]= _rowData;
                ...
                layui.each(data, function(i1, item1){
                    ...
                    that.eachCols(function(i3, item3){
                        ...
                        var colStatus=item3.hide===true?'class="cy-none"':"";
                        //td内容
                        var td = ['<td '+colStatus+'  data-field="'+ field +'" '+ function(){
                            var attr = [];
                            if(item3.edit) attr.push('data-edit="'+ item3.edit +'"'); //是否允许单元格编辑
                            if(item3.editUrl) attr.push('data-editUrl="'+ item3.editUrl +'"'); //编辑表格请求路径
                            ...
                            ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
                                ...
                                /**如果有枚举*/
                                if(item3.enumName){
                                    var enumValues = $t.getDataByEnum(item3.enumName).data || "";
                                    for (var _enum in enumValues) {
                                        if (enumValues[_enum].code == item1[field]) {
                                            item1[field] = enumValues[_enum].value||content;
                                        }
                                    }
                                }
                                ...
                                /**如果有字典*/
                                if(item3.codeName){
                                    var codeValues = $t.getDataByCode(item3.codeName).data || "";
                                    for (var _code in codeValues) {
                                        if (codeValues[_code].code == item1[field]) {
                                            item1[field] = codeValues[_code].value||content
                                        }
                                    }
                                }
                                /**如果有渲染方法*/
                                if(item3.render){
                                    item1[field]=eval((item3.render))(_rowData[i1],item1,i3,item1[field])||content;
                                }
                                //return item3.render ? eval((item3.render))(rowData,item1,i3,item1[field]) : item1[field];
    ...
    Class.prototype.sort = function(th, type, pull, formEvent){
        ...
        var that = this
            ...
            ,data = table.rowData[that.key], thisData;
